...
void print_text(int y, int x, string texts) {
    ...
    const char *text = new char[texts.length() + 1];// string to char* make
    mvprintw(y,x,"%s",text);
	refresh();
	echo();
}
...
void print_list(WINDOW *list_win)
{
    ...
    Node* p = l.first;
    while (p) {
        mvwprintw(list_win, y, x, "%s", p->val.c_str());
        y+=1;
        p = p->next;
    }
    wrefresh(list_win);
...
while(running)
{
	...
	print_list(list_win);
	mvwprintw(0, 0, "Use arrows to go up and down, Press enter to select. \n P.s. programm works ONLY with numbers".c_str());
	c = wgetch(menu_win);
	switch(c)
	{	case KEY_UP:
			if(highlight == 1)
